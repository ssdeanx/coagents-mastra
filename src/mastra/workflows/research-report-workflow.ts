...
import { z } from 'zod';
import { researchAgent, analyzerAgent, masterAgent, supervisorAgent } from '../agents/index';
...
const fileOperationOutputSchema = z.object({
  success: z.boolean().describe('Indicates if file write was successful'),
  message: z.string().describe('Details about the file write operation'),
  fileName: z.string().optional().describe('Name of the file created'),
});
...
/**
 * Step 2: Store Raw Research Data using Master Agent
 */
const storeResearchDataStep = createStep({
  id: 'store-raw-research-data',
  description: 'Stores the raw research findings in a file using the Master Agent.',
  ...
  outputSchema: fileOperationOutputSchema, // Use structured output schema
  execute: async ({ inputData }) => {
...
/**
 * Step 6: Store Final Report using Master Agent
 */
const storeFinalReportStep = createStep({
  id: 'store-final-report',
  description: 'Stores the final generated report in a file using the Master Agent.',
  inputSchema: z.object({
    topic: z.string(),
    finalReport: z.string(),
