...
import { z } from 'zod';
import { researchAgent, analyzerAgent } from '../agents/index';
import { PinoLogger } from '@mastra/loggers';
import { RuntimeContext } from '@mastra/core/runtime-context';
import { researchAgentOutputSchema } from '../agents/research-agent'; // Import researchAgentOutputSchema
 ...
 * If a documentPath is provided, use researchAgent to read the file.
...
const readDocumentStep = createStep({
  ...
  execute: async ({ inputData }) => {
    if (inputData.documentPath) {
      ...
      const content = resultObject.findings;
      if (typeof content !== 'string') {
        throw new Error(`Unexpected data format from researchAgent. Expected string, got ${typeof content}`);
      }
