...
export const codeSearchTool = createTool({
  ...
  execute: async ({ context }) => {
    ...
    const searchDirectory = context.directory || process.cwd(); // Default to current working directory
      ...
      const sanitizedPattern = escapeStringRegexp(context.regexPattern);
      ...
      for await (const filePath of walk(searchDirectory)) {
        filesScanned++;
        if (context.filePathPattern && !filePath.match(new RegExp(escapeStringRegexp(context.filePathPattern)))) {
          continue;
        }

        // Resolve and validate path to prevent unauthorized file access
        const absoluteFilePath = path.resolve(filePath);
        const rootDir = path.resolve(searchDirectory);
        if (!absoluteFilePath.startsWith(rootDir + path.sep)) {
          continue;
        }
